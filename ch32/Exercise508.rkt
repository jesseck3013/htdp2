...
(define (split-structural ed x)
  (cond
    ...
    [else
     (local (
             (define current-ed (split-structural
                                 (rest ed)
                                 x))
             (define current-pre (editor-pre current-ed))
             (define current-post (editor-post current-ed))
             (define one-more-on-pre (add-to-last
                                      (first ed)
                                      current-pre))
             )
       (if (>=
