...
(define-struct user [name email])

(define user1 (make-user "bob" "xx@mail.com"))

(check-expect (user-name user1) "bob")
(check-expect (user-email user1) "xx@mail.com")
(check-expect (user? user1) #true)
(check-expect (user? 11) #false)
