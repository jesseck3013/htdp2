  ...
  (local (
          ...
          (define (subtract-equations e equations)
            (cond
              [(empty? equations) '()]
              [else
               (cons (subtract e (first equations))
...
; 1.
; - trivial problem: (= (length soe) 1)

; 2. solution to the trivial problem
; soe itself

; 3. generate new problems:
; use (first soe) to subtract all (rest soe), and combine them as a new matrix

; 4. To combine the solution, (cons (first soe) (triangulate (rest soe)))
