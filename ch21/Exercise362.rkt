...
(define (interpreter sexpr sl)
  (local
      (
       (define expr (parse sexpr))

       (define defs
         (map parse sl))

       ; [X] [List-of X] -> BSL-da-all
       ; (make-BSL-da-all AL [List-of fun-def])
       (define da
         (make-BSL-da-all
          (get-def const-def? defs)
          (get-def fun-def? defs)
          ))
       )
  (eval-all expr da)))

(define WRONG "incorrect BSL syntax")
      ...
      [(and (= L 3) (symbol? (first s)))
       (cond
         ...
         [(and (symbol=? (first s) 'define)
               (symbol? (second s)))
          (make-const-def (second s) (third s))]
         [(and (symbol=? (first s) 'define)
               (list? (second s)))
          (local (
                  (define f-name
            ...
            (make-fun-def f-name (parse f-arg) (parse f-body)))]
         [else (error WRONG)])]
      [else (error WRONG)])))
...
; - fun-def
; - const-def
; [X] [X -> boolean] [List-of X] -> [List-of const-def]
(define (get-def type? lox)
  (cond
    [(empty? lox) '()]
    [(type? (first lox))
     (cons (first lox)
           (get-def type? (rest lox)))]
    [else (get-def type? (rest lox))]))
...
(check-within 
 (interpreter '(volume-of-10-cylinder 1)
              '(
                (define close-to-pi 3.14)
