...
; BT Number -> Symbol
(check-expect (search NONE 10) #false)
(check-expect (search bt1 30) #false)
(check-expect (search bt1 24) 'i)
(define (search bt n)
  (cond
    [(no-info? bt) #false]
    [else
     (local (
             (define left-result
               (search (node-left bt) n))
             (define right-result
               (search (node-right bt) n))
             )
     (cond
       [(equal? (node-ssn bt) n) (node-name bt)]
       [(boolean? left-result) right-result]
       [else left-result]))]))
