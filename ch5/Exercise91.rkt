...
; interpretation (make-VCat x h d) describes a virtual cat
; whose x-cordinate is x, hapiness is h, and direction is d.
...
(define (clock-tick-handler cat)
  (make-VCat
   (if (string=? (VCat-direction cat) "right")
        (+ (VCat-x cat) 3)
        (- (VCat-x cat) 3))
  (cond
  ...
  (cond
    [(>= (VCat-x cat) (image-width BACKGROUND)) "left"]
    [(<= (VCat-x cat) 0) "right"]
    [else (VCat-direction cat)])))

(check-expect (clock-tick-handler (make-VCat 10 100 "right"))
              (make-VCat 13 99.9 "right"))

(define (should-stop vcat)
