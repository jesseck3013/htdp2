...
(define (place-queens a-board n)
  (cond
    ...
    [else
     (local (
             (define available-p (find-open-spots a-board))
             )
       (place-queens/list a-board available-p n))]))

; Board [List-of QP] -> [Maybe [List-of QP]]
(define (place-queens/list a-board lo-qp n)
  (cond
    [(empty? lo-qp) #false]
    [else (local
              (
               (define possible-path
                 (place-queens
                  (add-queen a-board (first lo-qp))
                  (sub1 n)))
               )
            (if (false? possible-path)
...
; A Board is [List-of Posn]
; It contains only posn that is safe to place a queen.
; Here safe mean placing a queen does not threatent all existing queen on the board.

; N -> Board 
; creates the initial n by n board
(define (board0 n)
  (foldr append '() (build-list n (lambda (x)
                         (append (build-list n (lambda (y)
                                                 (make-posn x y))))))))
; Board -> [List-of QP]
...
(define (add-queen a-board qp)
  (filter (lambda (available-qp)
            (not (threatening? qp available-qp))) a-board))

; N -> Solution
(define (n-queens n)
  (place-queens (board0 n) n))

(check-satisfied (n-queens 4) (n-queens-solutions? 4))
