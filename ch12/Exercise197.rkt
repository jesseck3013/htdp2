...
; Sort the llc by count in a descending order.
(check-expect (sort>
               (list (make-LC "a" 10)
                     (make-LC "b" 20)
                     (make-LC "c" 30)))
...
(define (sort> llc)
  (cond
    [(empty? llc) '()]
    [else (insert (first llc)
                  (sort> (rest llc)))]))

; LC List-of-LC -> List-of-LC
(check-expect (insert (make-LC "a" 10) '())
              (list (make-LC "a" 10)))
...
; For this specific problem, I prefer the pick-max-count version, because it requires less code.
; However, in a more general programming environment, I think I don't need to implement the sorting function myself. I would prefer the sorting version in this situation.
