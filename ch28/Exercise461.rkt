...
(define (integrate-adaptive f a b)
  (local (
          ...
          (define (area-of-trapezoid l r fl fr)
            (* 1/2 (- r l) (+ fl fr)))
          (define current-area
            (area-of-trapezoid a b fa fb))
          (define left-half-area
            (area-of-trapezoid a mid fa fmid))
          (define right-half-area
            (area-of-trapezoid mid b fmid fb))
          (define diff (- current-area
                          (+ left-half-area
                             right-half-area)))
          )
    (cond
      [(<= diff (* ε (- b a))) current-area]
      [(<= (- b a) ε) (area-of-trapezoid a b fa fb)]
      [else (+ (integrate-adaptive f a mid)
               (integrate-adaptive f mid b))])))

(define integrate integrate-adaptive)
...
; The new method failed to pass the 3rd test. Its estimation is 1000.48828125.

; The new method is more suited for a curve whose slope does not change rapidly.
